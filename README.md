...
| **Redirect-to-variable** | `set x < file` assigns the first line of the file to `x` (tcsh PR #105). Reading must be asked for explicitly — a bare `set x` is an ordinary assignment and never touches standard input — and the redirection is the only form: as in every csh, a pipeline stage runs in a child process, so `echo foo \| set x` could not assign to `x` no matter what the shell read there. |
