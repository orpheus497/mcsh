...
| **Redirect-to-variable** | `set x < file` assigns the first line of the file to `x` (tcsh PR #105). Reading must be asked for explicitly: a bare `set x` is an ordinary assignment and never touches standard input. `echo foo \| set x` does read the pipe, but — as in every csh — the pipeline stage runs in a child process, so the variable it sets does not survive. |
...
| **System information panel** | `set sysinfo` | A `fastfetch`-style panel — login and host, OS, kernel, architecture, uptime, shell, terminal, CPU and core count, memory and root-filesystem use, load averages, a logo and the terminal palette. Printed by the `sysinfo` builtin on demand, and once at interactive start-up when the variable is set. Assembled from `uname(2)`, the password database, `sysconf(3)`, `statvfs(3)` and, where present, `/etc/os-release` and `/proc`; **no external program is run**, and a value that cannot be read is omitted rather than guessed. `sysinfo -n` drops the logo, as does a terminal too narrow for it, and a redirected panel is plain text. |

### Prompt

| Feature | Description |
|---------|-------------|
| **Git repository status** | `%v` shows `*n` modified tracked files, `!n` unmerged paths, `$n` stash entries and `^` when HEAD differs from its configured upstream; `%V` prepends the branch and operation state. Derived by parsing `.git/index` and comparing it against the working tree — the same stat comparison git's own fast path makes — plus the stash log, `config`, and the refs (honouring `packed-refs`). Still no `git` process. Computed lazily, so a prompt using only `%g` never pays for it. Measured 0.22 ms over 536 tracked files, per prompt — nothing cheaper can detect a tracked file edited in the working tree, because that edit touches no file inside `.git`. Scales linearly with the index, so `$GIT_POLL_INTERVAL` is there to throttle it in very large repositories; above 20000 index entries it is not scanned at all. |
| **Native git branch** | `%g` expands to the current branch name (or the 7-character object name on a detached `HEAD`); `%G` also appends the operation state (`main\|MERGING`, `main\|REBASING-i`, `main\|BISECTING`, `abc1234\|DETACHED`, …). Both are empty outside a git repository. No `git` process is spawned — the control files are read directly. Works from any subdirectory, and in linked worktrees, submodules, and bare repos. Cached per-CWD against the resolved git directory: HEAD is compared by contents (exact — `st_mtime` has one-second granularity) and the state markers by mtime, tracked independently so merges, rebases and cherry-picks are detected promptly without false refreshes. Re-checked on **every prompt** by default, so the branch and state shown are the ones the command just run left behind: the prompt is rendered once per command, so withholding the re-check saves no repeated work — it only withholds the result the reader is looking at. A re-check that finds nothing changed costs one `open`/`read`/`close` of `HEAD` and one `stat(2)` of each of eight markers, measured at 17 µs per prompt. `$GIT_POLL_INTERVAL` sets a minimum interval in seconds for anyone who wants the old throttle; 0 (the default) means every prompt. |
