...
| **System information panel** | **`set sysinfo`** *(and the `sysinfo` builtin)* | **`tc.fetch.c`, `sh.init.c`, `sh.c`, `tc.const.c`, `tc.decls.h`** | fastfetch-style panel: identity, OS, kernel, arch, uptime, shell, terminal, CPU, memory, root filesystem, load, logo and palette. Read with `uname(2)`, the password database, `sysconf(3)`, `statvfs(3)`, `/etc/os-release` and `/proc`; no process spawned, and an unreadable field is omitted rather than approximated. `sysinfo -n` drops the logo; colour only on a colour-capable tty. |
